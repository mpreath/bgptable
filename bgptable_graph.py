...
if len(rows) > 1:
    ...
    for row in rows:
        ...
        if(weeknum == c_weeknum):
            ...
            bgp4_day_list[dow-1] = row[1]
            bgp4_xlabels[dow-1] = str(dt.date())
            if(row[1] > bgp4_day_max[dow-1]):
                bgp4_day_max[dow-1] = row[1]

            if(row[1] < bgp4_day_min[dow-1] or bgp4_day_min[dow-1] == 0):
                bgp4_day_min[dow-1] = row[1]
...
if len(rows) > 1:
    ...
    for row in rows:
        ...
        if(weeknum == c_weeknum):
            ...
            week_last_value = row[1]
            bgp6_day_list[dow-1] = row[1]
            bgp6_xlabels[dow-1] = str(dt.date())
            if(row[1] > bgp6_day_max[dow-1]):
                bgp6_day_max[dow-1] = row[1]

            if(row[1] < bgp6_day_min[dow-1] or bgp6_day_min[dow-1] == 0):
                bgp6_day_min[dow-1] = row[1]
